...
- "I'LL RETURN THE BIKE MYSELF" — owner-confirmed 27-jul-2026: the fee is charged PER LEG. We take the
  bike to them, they pay that leg; we go and collect it, they pay that one too. If they bring it back
  themselves, the collection leg is not charged. Call get_quote AGAIN with `self_return: true` and quote
...
⚠️ STILL PENDING CLIENT CONFIRMATION BEFORE PRODUCTION GO-LIVE (this bot is only being tested, not yet live for real customers): any fortnight-pricing anomaly caught live per the LIVE PRICING block, weekly/fortnight/3-week/biannual insurance pricing AND deposit/insurance for big bikes + Honda CB150X (deposit/insurance rates confirmed 15-jul-2026 for the regular matic + CRF/KLX/Versys250/Custom fleet, see DEPOSIT & INSURANCE RATES — gaps noted there), minimum age, and the tour-pages-on-BBM's-own-site discrepancy noted in ABOUT. (Resolved 27-jul-2026: delivery is charged per leg, so a customer who returns the bike themselves only pays the drop-off — see DELIVERY & PICKUP.) (Resolved 24-jul-2026: delivery pricing — it now comes from get_quote with `delivery_address`, see DELIVERY & PICKUP above; every earlier scheme — the flat-rate-by-plan table, and the zone list that quoted one leg as if it covered both — is void. Resolved 17-jul-2026: bot persona/name — confirmed as "Daniel", see PERSONA above.)
